...
[Step 3] 공공데이터 연계 매핑 결과(ec_mapped_with_api.csv)를 기반으로
맞춤 큐레이션 및 영양제 검색/비교 시 실물 상품 이미지(img_url)를 함께 시각화합니다.
"""
...
def load_data():
    ...
    df['img_url'] = df['img_url'].fillna(DEFAULT_IMG)
...
def main():
    ...
    # ==========================================
    # [탭 1] 스마트 문진 및 AI 진단 (이미지 포함 추천 카드)
    # ==========================================
    with tab1:
        ...
        with agree_cols[0]:
            agree_terms = st.checkbox("[필수] 서비스 이용약관 및 일반 개인정보 수집·이용 동의")
        with agree_cols[1]:
            agree_age = st.checkbox("[필수] 만 14세 이상 이용 확인 (만 14세 미만 제한)")
        with agree_cols[2]:
            agree_health = st.checkbox("[필수] 건강 상태 및 라이프스타일(민감정보) 수집·이용 동의")
        ...
        if not all_agreed:
            ...
            with c1:
                gender = st.radio("1-1. 성별", ["남성", "여성", "응답하지 않음"], horizontal=True, key="gender_radio_img")
                male_concerns = []
                female_status = "해당 없음"
                if gender == "남성":
                    male_concerns = st.multiselect("남성 특화 추가 고민 선택", ["전립선 건강", "남성형 탈모 고민", "근육량 유지/운동"], key="male_concerns_img")
                elif gender == "여성":
                    female_status = st.radio("여성 생애주기/상태 선택", ["해당 없음", "임신 준비 중", "임신 중", "수유 중", "폐경기"], key="female_status_radio_img")
                    
            with c2:
                age_group = st.selectbox("1-2. 연령대", ["20대 미만", "20대", "30대", "40대", "50대 이상"], index=2)
                
            with c3:
                height = st.number_input("1-3. 키 (cm)", min_value=100.0, max_value=230.0, value=160.0, step=0.5, key="height_input_img")
                weight = st.number_input("몸무게 (kg)", min_value=30.0, max_value=200.0, value=50.0, step=0.5, key="weight_input_img")
                
                height_m = height / 100.0
                bmi = weight / (height_m * height_m)
                
                if bmi < 18.5:
                    bmi_status = "저체중 (체중 관리 필요)"
                    bmi_color = "#0284c7"
                elif 18.5 <= bmi < 23.0:
                    bmi_status = "정상 체중"
                    bmi_color = "#16a34a"
            ...
            if diagnose_click:
                ...
                st.success(f"✅ 총 {len(df):,}개 영양제 중 안전성 및 제형 검증 후 {len(df_filtered):,}개 적합 제품군이 선별되었습니다.")
                if filter_reasons:
                    with st.expander("🛡️ 적용된 안전 및 제형 필터 세부 내역 확인"):
                        for r in filter_reasons: st.write(f"• {r}")

                final_recommendations = []
                if len(health_goals) == 0:
                    top3 = df_filtered.sort_values(by='popularity_score', ascending=False).head(3)
                    for _, r in top3.iterrows():
                        final_recommendations.append(("종합 추천", r))
                else:
                    ...
                    for goal in health_goals:
                        ...
                        def goal_score(row):
                            ...
                            f_raw = str(row['functionality_raw']).lower()
                            for ing in target_ings:
                                if ing.lower() in p_name or ing.lower() in m_ing or ing.lower() in f_raw:
                                    score += 20.0
                            price = row['price']
                            if budget_range == "3만원 이하" and price <= 30000: score += 5.0
                            elif budget_range == "3만원~5만원" and 30000 < price <= 50000: score += 5.0
                            elif budget_range == "5만원~10만원" and 50000 < price <= 100000: score += 5.0
                            elif budget_range == "10만원 이상" and price > 100000: score += 5.0
                        ...
                        for _, r in df_goal_top.iterrows():
                            final_recommendations.append((goal, r))
                            used_product_ids.add(r['product_id'])
                            
                st.markdown("### 🥇 고민 영역별 맞춤 영양제 다각화 추천 (실물 이미지 연동)")
                
                if not final_recommendations:
                    st.warning("⚠️ 선택하신 조건에 만족하는 영양제가 없습니다.")
                else:
                    cols = st.columns(len(final_recommendations))
                    for idx, (g_name, row) in enumerate(final_recommendations):
                        ...
                        with cols[idx]:
                            ...
                            <div class="top-product-card">
                                <div>
                                    <img src="{img_src}" class="product-img-box" alt="영양제 이미지"/>
                                    <span class="badge-goal">🎯 고민: {g_name}</span>
                                    <span class="badge-platform">{row['platform'].upper()}</span>
                                    <span class="badge-green">{row['form_type']}</span>
                                    <h4 style="margin-top:0.6rem; color:#0e462d; font-size:1.05rem; min-height: 2.6rem;">{row['product_name']}</h4>
                                    ...
                                    <p style="font-size:0.85rem; color:#333;"><b>🧪 주요 기능 원료:</b><br/>{row['matched_ingredient']}</p>
                                    <p style="font-size:0.8rem; color:#666;"><b>💡 식약처 인증 기능성:</b><br/>{str(row['functionality_raw'])[:45]}...</p>
                                </div>
                                <div>
    ...
    # ==========================================
    with tab2:
        st.subheader("🏷️ 카테고리별 인기 영양제")
        st.caption("20종 이상의 세분화된 원료별 이커머스 통합 인기 스코어[review_count * rating] 순위입니다.")
        ...
        selected_cat = st.selectbox("🎯 조회할 영양 성분 카테고리를 선택하세요", available_categories, key="cat_select_v3")
        ...
        if df_cat_top10.empty:
            ...
            with c2:
                st.markdown("##### 📄 TOP 10 상세 리스트 (구매 링크 제공)")
                renamed_df = df_cat_top10[['brand', 'product_name', 'price', 'rating', 'review_count', 'platform']].copy().reset_index(drop=True)
                renamed_df.columns = ['브랜드', '상품명', '가격(원)', '평점', '리뷰 수', '플랫폼']
                st.dataframe(renamed_df.style.format({'가격(원)': '{:,.0f}', '평점': '{:.1f}', '리뷰 수': '{:,.0f}'}), use_container_width=True, height=430)
    ...
    with tab3:
        ...
        st.caption("연령대별 필요 영양소 가중치를 적용한 맞춤형 랭킹 분석 결과입니다.")
        
        selected_age = st.radio("조회할 연령대를 선택하세요", ["20대", "30대", "40대", "50대", "60대 이상"], horizontal=True, key="age_radio_v3")
        
        age_weights = {
            "20대": ["비타민 B군·비오틴(에너지·활력)", "프로바이오틱스(유산균/장 건강)", "비타민 C(항산화·면역)", "콜라겐(피부 탄력)"],
            "30대": ["비타민 B군·비오틴(에너지·활력)", "rTG 오메가-3(혈관·혈행)", "마그네슘(신경·근육)", "프로바이오틱스(유산균/장 건강)"],
            "40대": ["rTG 오메가-3(혈관·혈행)", "밀크씨슬(실리마린/간 건강)", "루테인·지아잔틴(눈 건강)", "비타민 D(뼈 건강·면역)"],
            "50대": ["rTG 오메가-3(혈관·혈행)", "코엔자임Q10(항산화·혈압)", "콘드로이친(관절 건강)", "MSM·글루코사민(관절/연골)", "루테인·지아잔틴(눈 건강)"],
            "60대 이상": ["콘드로이친(관절 건강)", "MSM·글루코사민(관절/연골)", "칼슘(뼈·치아)", "비타민 D(뼈 건강·면역)", "아연(면역 기능)"]
        }
        target_ing_list = age_weights.get(selected_age, [])
        
        def calc_age_score(row):
            score = row['popularity_score']
            if row['matched_ingredient'] in target_ing_list:
                score *= 2.5
            return score
            
        df_age = df.copy()
        df_age['age_score'] = df_age.apply(calc_age_score, axis=1)
        df_age_top10 = df_age.sort_values(by='age_score', ascending=False).head(10)
        
        st.info(f"💡 **{selected_age} 주요 추천 집중 성분**: {', '.join(target_ing_list)}")
        
        a1, a2 = st.columns([1.2, 1])
        with a1:
            fig_age = px.bar(
                df_age_top10.sort_values(by='age_score', ascending=True),
                x='age_score', y='product_name', orientation='h',
                title=f"🎈 {selected_age} 맞춤 인기 스코어 TOP 10",
                labels={'age_score': '연령 가중 인기 점수', 'product_name': '상품명'},
                color='age_score', color_continuous_scale='Viridis'
            )
            fig_age.update_layout(height=480, showlegend=False, margin=dict(l=20, r=20, t=40, b=20), font=dict(family="Pretendard, sans-serif"))
            st.plotly_chart(fig_age, use_container_width=True)
        with a2:
            st.markdown(f"##### 📋 {selected_age} 대표 영양제 추천 목록")
            display_age_df = df_age_top10[['brand', 'product_name', 'matched_ingredient', 'price', 'platform']].reset_index(drop=True)
            display_age_df.columns = ['브랜드', '상품명', '주요 성분', '가격(원)', '출처']
            st.dataframe(display_age_df.style.format({'가격(원)': '{:,.0f}'}), use_container_width=True, height=430)

    # ==========================================
    # [탭 4] 영양제 시너지 & 복용 타임 가이드
    # ==========================================
    with tab4:
        st.subheader("💊 영양제 복용 시너지 조합 및 섭취 골든타임 가이드")
        st.caption("영양제는 함께 먹는 조합과 섭취 시간대에 따라 흡수율과 효과가 극대화됩니다.")
        ...
        with g1:
            ...
            <div class="custom-card" style="border-left: 5px solid #16a34a;">
                <h5 style="color:#16a34a; margin-bottom:0.3rem;">3. 비타민 C + 콜라겐</h5>
                <p style="font-size:0.95rem;">비타민C는 체내에서 콜라겐이 합성되는 과정에 필수적인 조효소로 작용하여 콜라겐 생성을 활성화합니다.</p>
            </div>
            <div class="custom-card" style="border-left: 5px solid #16a34a;">
                <h5 style="color:#16a34a; margin-bottom:0.3rem;">4. 유산균 + 프리바이오틱스 (신바이오틱스)</h5>
                <p style="font-size:0.95rem;">유익균(유산균)과 유익균의 먹이(프리바이오틱스)를 함께 섭취하면 장내 정착률과 증식 속도가 높아집니다.</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with g2:
            ...
            <div class="custom-card" style="border-left: 5px solid #dc2626;">
                <h5 style="color:#dc2626; margin-bottom:0.3rem;">2. 종합비타민 vs 아연/철분 고함량</h5>
                <p style="font-size:0.95rem;">종합비타민에 이미 기본 미네랄이 들어있어 고함량 단일 미네랄을 추가 섭취 시 위장 장애나 과다증을 유발할 수 있습니다.</p>
            </div>
            <div class="custom-card" style="border-left: 5px solid #dc2626;">
                <h5 style="color:#dc2626; margin-bottom:0.3rem;">3. 오메가3 vs 키토산</h5>
                <p style="font-size:0.95rem;">키토산은 지방을 흡착하여 배출하는 성질이 있어 오메가3의 유용한 지방산까지 흡착 배출시킵니다.</p>
            </div>
            """, unsafe_allow_html=True)
            
        st.divider()
        st.markdown("#### ⏰ 하루 섭취 골든타임 시계표")
        t1, t2, t3, t4 = st.columns(4)
        with t1: st.info("🌅 **아침 공복 (식전 30분)**\n- 유산균 (위산 노출 최소화)\n- 비타민 B군 (에너지 대사 활성화)")
        with t2: st.success("☀️ **점심 식후 바로**\n- 종합비타민 / 비타민 C\n- 오메가3 (지방 소화액과 함께 흡수)")
        with t3: st.warning("🌆 **저녁 식후 바로**\n- 루테인 / 지아잔틴\n- 코엔자임Q10 / 밀크씨슬")
        with t4: st.error("🌙 **취침 전 1시간**\n- 마그네슘 (신경 안정 및 수면 도모)\n- 칼슘 (야간 뼈 손실 방지)")

    # ==========================================
    # [탭 5] 영양제 검색 & 스펙 비교 (이미지 렌더링 포함)
    # ==========================================
    with tab5:
        st.subheader("🔍 전체 28,239개 영양제 다중 키워드 검색 & 1:1 제품 비교")
        st.caption("공백 단위 멀티 키워드 검색 지원 (예: '종근당 비타민', '고려은단 C', '오쏘몰 7일분')")
        
        search_query = st.text_input("🔎 검색어 입력 (띄어쓰기로 여러 단어 검색 가능)", value="종근당 비타민", key="search_query_v3")
        ...
        if search_query.strip():
            ...
            if not search_df.empty:
                st.dataframe(
                    search_df[['platform', 'brand', 'product_name', 'price', 'rating', 'review_count', 'matched_ingredient', 'form_type']].reset_index(drop=True),
                    use_container_width=True,
                    height=320
                )
                
                st.markdown("#### ⚖️ 검색 결과 중 최대 3개 제품 1:1 스펙 & 실물 이미지 비교")
                product_options = search_df['product_name'].unique()[:30]
                selected_products = st.multiselect("비교할 제품을 선택하세요 (최대 3개)", product_options, max_selections=3, key="comp_select_v3")
                    ...
                    for idx, (_, row) in enumerate(comp_df.iterrows()):
                        ...
                        with comp_cols[idx]:
                            ...
                            <div class="custom-card" style="text-align:center;">
                                ...
                                <h5 style="margin-top:0.5rem; min-height: 2.4rem;">{row['product_name']}</h5>
                                <p style="margin-bottom:0.3rem;"><b>브랜드:</b> {row['brand']} | <b>출처:</b> {row['platform'].upper()}</p>
